...
object DummyBackend {
   ...
   class DummyModelBackend extends ModelBackend {
     override def save[A <: ModelEntityDefinition : ClassTag](model: A): Future[A] = Future {
       model
     }
   ...
   class DummyContentObjectBackend extends ContentObjectBackend {
   ...
   implicit val dummyContentObjectBackend = new DummyContentObjectBackend
 }