...
package object backend {
  ...
  class InMemoryModelBackend extends ModelBackend {
    /** registered models */
    private var modelDefinitions = Map[HyperdocRef, ModelEntityDefinition]()

    override def save[A <: ModelEntityDefinition : ClassTag](obj: A): Future[A] = Future {
    ...
    override def get[A <: ModelEntityDefinition : ClassTag](ref: HyperdocRef): Future[Option[A]] = Future {
  ...
  class InMemoryContentObjectBackend extends ContentObjectBackend {
  ...
  implicit val inMemoryContentObjectBackend = new InMemoryContentObjectBackend
}